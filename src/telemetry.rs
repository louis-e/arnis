...
pub fn redact_url_queries(mut input: &str) -> String {
    ...
    while let Some(scheme_idx) = input.find("://") {
        ...
        // XYZ tile URLs encode the user's location in the PATH
        // (/{z}/{x}/{y}), not the query — truncate known tile services
        // to their host so a tile-fetch error message can never ship
        // coordinates. The host alone still identifies which provider
        // failed, which is all telemetry needs.
...
mod tests {
    ...
    fn redact_truncates_tile_url_paths() {
        // XYZ tile paths encode the user's location — must not survive.
