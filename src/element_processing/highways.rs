...
pub fn generate_highways(
    ...
    if let Some(highway_type) = element.tags().get("highway") {
        if highway_type == "street_lamp" {
        ...
        } else if element.tags().get("area").map_or(false, |v: &String| v == "yes") {
            ...
            let polygon_coords: Vec<(i32, i32)> = way.nodes.iter().map(|n: &crate::osm_parser::ProcessedNode| (n.x, n.z)).collect();
