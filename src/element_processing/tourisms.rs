use crate::block_definitions::{COBBLESTONE_WALL, OAK_PLANKS};
use crate::osm_parser::ProcessedNode;
use crate::world_editor::WorldEditor;

pub fn generate_tourisms(editor: &mut WorldEditor<'_>, element: &ProcessedNode) {
    ...
    if let Some(tourism_type) = element.tags.get("tourism") {
        ...
        if tourism_type == "information"
            && let Some(info_type) = element.tags.get("information").map(|x: &String| x.as_str())
            && info_type != "office"
            && info_type != "visitor_centre"
        {
